...
- [`docs/contracts/agent-phone-interface.md`](docs/contracts/agent-phone-interface.md) defines the local-only agent-facing phone UI contract, including the allowed visual verbs, `assistant.sock`, `ai.sock`, protected modes, `/mobile_capabilities`/`/mobile_state` discovery, the validated `hestia-mobile-agent` adapter CLI, and the `hestia-mobile-fake-phone` offline harness.
